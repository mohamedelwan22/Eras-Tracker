...
import { Calendar, Globe, ExternalLink, ArrowRight, MapPin } from 'lucide-react';
...
export function SearchResultCard({ event, index }: SearchResultCardProps) {
    ...
    const hasExternalSource = event.sources && event.sources.length > 0 && event.sources[0].url;
    ...
    return (
        ...
        >
            <CardWrapper>
                <div className="group grid grid-cols-1 md:grid-cols-12 bg-card rounded-2xl overflow-hidden border border-border/50 hover:border-primary/30 transition-all duration-300 hover:shadow-2xl hover:shadow-primary/5 h-full">
